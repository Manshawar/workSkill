...
function loadUiMap() {
  const ui = loadJson(
    [path.join(SKILL_DIR, 'ui.local.json'), path.join(SKILL_DIR, 'ui.example.json')],
    'ui',
  );
  if (!ui) {
    console.error('❌ missing ui map: copy ui.example.json → ui.local.json and fill page labels');
    process.exit(2);
  }
  const required = ['tabAccount', 'phAccount', 'phVerify', 'orgPrompt', 'btnSubmit', 'btnNext', 'btnBackend'];
  for (const k of required) {
    if (!ui[k] || ui[k] === '') {
      console.error(`❌ ui map missing key: ${k} (fill ui.local.json)`);
      process.exit(2);
    }
  }
  return ui;
...
(async () => {
  ...
  console.log('  verify   :', CONFIG.verifyToken);
